...
<html>
	...
	<body>
		<div class="content-box">
			 <br>
			 <button class="layui-btn" id="addScene">新增年龄段</button>
		...
		<script type="text/javascript">
			layui.use(['table','jquery','element','form','layer'], function(){
			 ...
			 //form.render();
			  ...
			  $("#addScene").on('click',function(){
				  var url ="${basePath}/thesaurus/ageBand/ageBand-add";
		            layer.open({
		                type: 2,
		                area: ['450px', '280px'],
		                offset: ['120px', '450px'],
			...
			//delte删除大分类
			 function remove(id,action){
			 	var url ="${basePath}/thesaurus/ageBand/ageBand-delete";
		        layer.confirm('确认删除', {
		            btn: ['删除','取消'],icon: 7, title:'注意' //按钮
		        }, function(index){
		            $.post(url,{"id":id,"ageBand":action},function (data) {
		                if(data.isSuccess){
		                    layer.msg(data.message);
		                    var pageNum = $("#pageNum").val();
		                    jump("${basePath}/thesaurus/ageBand/ageBand-find",pageNum);
		                }else{
		                	  layer.msg(data.message);
		                }
		            })
		    ...
		    function _returnData(pageNum) {
		        var url = "${basePath}/thesaurus/ageBand/ageBand-find";
		        jump(url,pageNum);
		    }
		</script>
	</body>
