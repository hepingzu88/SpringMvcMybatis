...
<html>
	...
	<body>
		<div class="content-box">
			 <br>
			 <button class="layui-btn" id="addScene">创建场景</button>
		...
		<script type="text/javascript">
			...
			//delte删除大分类
			 function remove(id,scene){
			 	var url ="${basePath}/thesaurus/custom-scene-delete";
		        layer.confirm('确认删除', {
		            btn: ['删除','取消'],icon: 7, title:'注意' //按钮
		        }, function(index){
		            $.post(url,{"id":id,"scene":scene},function (data) {
		                if(data.isSuccess){
		                    layer.msg(data.message);
		                    var pageNum = $("#pageNum").val();
		                    jump("${basePath}/thesaurus/custom/custom-find",pageNum);
		                }else{
		                	layer.msg(data.message);
		                }
		            })
		    ...
		    function _returnData(pageNum) {
		        var url = "${basePath}/thesaurus/custom/custom-find";
		        jump(url,pageNum);
		    }
		</script>
	</body>
