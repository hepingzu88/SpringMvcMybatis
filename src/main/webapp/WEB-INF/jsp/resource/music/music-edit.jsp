...
<html>
	...
	<body>
		...
		<form class="layui-form layui-form-pane" style="margin-left: 10px;">
		  <input type="hidden" id="musicResId" value="${musicResId}">
		    ...
		    <div class="layui-input-block">
		      <select name="ageId" id="ageId" lay-filter="age" lay-search lay-verify="required" >
		        <c:forEach items="${listAge}" var="item">
		        	<c:choose>
						<c:when test="${musicRes != null && musicRes.ageId == item.id}">
		  ...
		  form.render();
		   ...
		   form.on('submit(from)',function(data){
			   var text = $("input:checkbox[name='arr']:checked").map(function(index,elem) {
				 ...
				 console.log(text);
	    	    $.post("${basePath}/music/edit",{"ageId":$("#ageId").val(),"tagArr":text,"musicResId":$("#musicResId").val()},function (res) {
	    		   if(res.isSuccess){
		   ...
		   $(function(){
			   ...
			   $.post('${basePath}/music/queryMusicResTag',{"musicResId":$("#musicResId").val()},function(res){
					var roleArr = [];
